...
<template>
    <div class="container mx-auto p-6">
        ...
        <div class="mb-4 bg-white rounded-lg shadow p-4">
            <div class="grid grid-cols-1 2xl:grid-cols-4 gap-4 items-end">
                <!-- 地域多选 -->
                <!-- <div>
                ...
                <div class="flex items-end gap-2 2xl:col-span-1">
                        ...
                        <tr v-for="item in this.tableData" :key="item.id" class="hover:bg-gray-50"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-sm text-gray-500">
                                    ...
                                    <template v-if="item.rawData.remark">
                                        {{ item.rawData.remark.length > 18 ? item.rawData.remark.substring(0, 18) + '..' : item.rawData.remark }}
                                    </template>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <span @click="openEventsDialog(item)" :class="getEventClass(item.events)"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div v-if="this.monitorMetrics[item.rawData.host]" class="text-sm font-medium"
                                    style="width:67px">
                                   CPU 
                                    ...
                                    <!-- {{ this.monitorMetrics[item.rawData.host]?.loadStr || '-' }} -->
                                    <br> 内存

                                    <template v-if="this.monitorMetrics[item.rawData.host]?.MemTotal">
                                        <span :style="{
                                            color: 'hsl(' +
                                                (100 - (((this.monitorMetrics[item.rawData.host]?.MemTotal - this.monitorMetrics[item.rawData.host]?.MemAvail) / this.monitorMetrics[item.rawData.host]?.MemTotal
                                                    * 100)))
                                                + ', 100%, 35%)'
                                        }">
                                            {{ ((this.monitorMetrics[item.rawData.host]?.MemTotal -
                                                this.monitorMetrics[item.rawData.host]?.MemAvail) /
                                                this.monitorMetrics[item.rawData.host]?.MemTotal
                                                * 100).toFixed(1) }}%
                                        </span>
                                    </template>
